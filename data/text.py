...
class CharacterTextEncoder(_BaseTextEncoder):
    def __init__(self, vocab_list):
        ...
        # <pad>=0, <sos>=1, <unk>=2, <eos>=3
        self._vocab_list = ["<pad>", "<sos>", "<unk>", "<eos>"] + vocab_list
    ...
    def encode(self, s):
        ...
        return [[self.sos_idx] + [self.vocab_to_idx(v) for v in s], [self.vocab_to_idx(v) for v in s] + [self.eos_idx]]

    def decode(self, idxs, target = False, ignore_repeat=False):
        vocabs = []
        new_idxs_1 = []
        new_idxs_2 = []
        if not target:
            for t, idx in enumerate(idxs):
                if idx == self.pad_idx or (t > 0 and idx == idxs[t-1]):
                    continue
                else:
                    new_idxs_1.append(idx)

            for t, idx in enumerate(new_idxs_1):
                if idx == 38:
                    continue
                else:
                    new_idxs_2.append(idx)
            idxs = new_idxs_2
