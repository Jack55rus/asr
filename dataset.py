...
from data.text import load_text_encoder
from data.audio import create_transform
...
def collect_audio_batch(batch, audio_transform, mode, classification=False):
    ...
    is_match = []
    # Read batch
    file, audio_feat, audio_len, text_in, text_out = [], [], [], [], []
    with torch.no_grad():
        for i, b in enumerate(batch):
            ...
            # text_in.append(torch.LongTensor(b[1][0]))
            # text_out.append(torch.LongTensor(b[1][1]))
            ####
            file.append(str(batch[i][0]).split('/')[-1].split('.')[0])
            feat = audio_transform(str(batch[i][0]))
            audio_feat.append(feat)
            audio_len.append(len(feat))
            if classification and uniform(0, 1) > 0.7: # pick incorrect text 
                new_i = list(range(len(batch)))
                new_i.remove(i)
                i = choice(new_i)
                is_match.append(0)
                # text_in.append(torch.ones_like(torch.FloatTensor(batch[i][1][0])).type(torch.LongTensor))
            else:
                is_match.append(1)
            text_in.append(torch.LongTensor(batch[i][1][0]))
            text_out.append(torch.LongTensor(batch[i][1][1]))
    # Descending audio length within each batch
    audio_len, file, audio_feat, text_in, text_out = zip(*[(feat_len, f_name, feat, txt_in, txt_out)
                                              for feat_len, f_name, feat, txt_in, txt_out in sorted(zip(audio_len, file, audio_feat, text_in, text_out), reverse=True, key=lambda x:x[0])])
    # Zero-padding
    audio_feat = pad_sequence(audio_feat, batch_first=True)
    text_in = pad_sequence(text_in, batch_first=True)
    text_out = pad_sequence(text_out, batch_first=True)
    audio_len = torch.LongTensor(audio_len)

    return file, audio_feat, audio_len, text_in, text_out, torch.FloatTensor(is_match)
...
def load_dataset(n_jobs, use_gpu, pin_memory, ascending, corpus, audio, text):
    ...
    # print('audio_transform', audio_transform)
    # input('stop')
    # Text tokenizer
    ...
    collect_tr = partial(collect_audio_batch,
                         audio_transform=audio_transform, mode=mode, classification=True)
    collect_dv = partial(collect_audio_batch,
                         audio_transform=audio_transform, mode='test', classification=True)
    # print(collect_tr)
    # input('stop')
